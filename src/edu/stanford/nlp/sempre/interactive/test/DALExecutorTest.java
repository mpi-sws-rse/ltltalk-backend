...
import edu.stanford.nlp.sempre.interactive.robolurn.Color;
import edu.stanford.nlp.sempre.interactive.robolurn.Color.BasicColor;
import edu.stanford.nlp.sempre.interactive.robolurn.RoboAction;
...
public class DALExecutorTest {
  ...
  protected static void runFormula(DALExecutor executor, String formula, ContextValue context,
      Predicate<List<RoboAction>> checker) {
    LogInfo.begin_track("formula: %s", formula);
    DALExecutor.opts.worldType = "RoboWorld";
    ...
    if (checker != null) {
//      if (!checker.test(RoboAction.fromJSONObject(Json.readValueHard(jsonStr, List.class)))) {
//        LogInfo.end_track();
//        Assert.fail(jsonStr);
//      }
    }
  ...
  public Predicate<List<RoboAction>> checkColors(Map<Color.BasicColor, Integer> target) {
    return x -> {
      for (RoboAction ra : x) {
        if (ra.action == RoboAction.Action.PICKITEM && ra.possible == true) {
          if (target.containsKey(ra.spec)) {
            target.put(ra.spec, new Integer(target.get(ra.spec) - 1));
          } else {
            LogInfo.logs("Picked item with unexpected color %s", ra.spec.toString());
            return false;
          }
        }
      }
      
      for (Entry<BasicColor, Integer> e : target.entrySet()) {
        if (e.getValue() != 0) {
          LogInfo.logs("Number of items off by %d", e.getValue());
          return false;
        }
      }
      
//      LogInfo.logs("Got %d, expected %d", carried, n);
      return true;
    };
  }
...
//    String defaultBlocks = "[[1,1,1,\"Green\",[]],[1,2,1,\"Blue\",[]],[2,2,1,\"Red\",[]],[3,2,2,\"Yellow\",[]]]";
    String mapString = "[[0,-2,[]],[-5,-5,\"wall\",null],[-4,-5,\"wall\",null],[-3,-5,\"wall\",null],[-2,-5,\"wall\",null],[-1,-5,\"wall\",null],[0,-5,\"wall\",null],[1,-5,\"wall\",null],[2,-5,\"wall\",null],[3,-5,\"wall\",null],[4,-5,\"wall\",null],[5,-5,\"wall\",null],[5,-4,\"wall\",null],[5,-3,\"wall\",null],[5,-2,\"wall\",null],[5,-1,\"wall\",null],[5,0,\"wall\",null],[5,1,\"wall\",null],[5,2,\"wall\",null],[5,3,\"wall\",null],[5,4,\"wall\",null],[5,5,\"wall\",null],[4,5,\"wall\",null],[3,5,\"wall\",null],[2,5,\"wall\",null],[1,5,\"wall\",null],[0,5,\"wall\",null],[-1,5,\"wall\",null],[-2,5,\"wall\",null],[-3,5,\"wall\",null],[-4,5,\"wall\",null],[-5,5,\"wall\",null],[-5,4,\"wall\",null],[-5,3,\"wall\",null],[-5,2,\"wall\",null],[-5,1,\"wall\",null],[-5,0,\"wall\",null],[-5,-1,\"wall\",null],[-5,-2,\"wall\",null],[-5,-3,\"wall\",null],[-5,-4,\"wall\",null],[-5,-5,\"wall\",null],[-4,0,\"wall\",null],[-3,0,\"wall\",null],[-2,0,\"wall\",null],[-1,0,\"wall\",null],[2,0,\"wall\",null],[3,0,\"wall\",null],[4,0,\"wall\",null],[4,3,\"item\",\"blue\"],[4,-4,\"item\",\"red\"],[0,-2,\"item\",\"red\"],[0,-2,\"item\",\"green\"],[-3,3,\"item\",\"green\"]]";
    ContextValue context = getContext(mapString);
    LogInfo.begin_track("testPick");

    Map<Color.BasicColor, Integer> map = new HashMap<>();
    map.put(Color.BasicColor.Red, 1);
    
    runFormula(executor, "(: pick (number 1) (color red))", context, checkColors(map));
