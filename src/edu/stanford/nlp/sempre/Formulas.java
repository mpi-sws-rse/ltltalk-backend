...
public abstract class Formulas {
	public static Formula fromLispTree(LispTree tree) {
		...
		if (func != null) {
			...
			if (func.equals("limit"))
				return new LimitFormula(fromLispTree(tree.child(1)), fromLispTree(tree.child(2)));
			if (func.equals("mark"))
		...
		{ // Merge: (and (fb:type.object.type fb:people.person) (fb:people.person.children
			// fb:en.barack_obama))
		...
		{ // Superlative: (argmax 1 1 (fb:type.object.type fb:people.person) (lambda x
			// (!fb:people.person.height_meters (var x))))
			...
			if (mode != null) {
				...
				return new SuperlativeFormula(mode, rank, count, fromLispTree(tree.child(3)),
						fromLispTree(tree.child(4)));
			}
		}

		{ // Arithmetic: (- (!fb:people.person.height_meters (var x))
			// (!fb:people.person.height_meters (var y)))
	...
	// Special case to enable "argmax 1 1" rather than "argmax (number 1) (number
	// 1)"
	...
	// Replace top-level occurrences of |searchFormula| inside |formula| with
	// |replaceFormula|.
	...
	// TODO(joberant): use Formula.map, and use CanonicalNames.isReverseProperty,
	// etc.
	...
	// TODO(jonathan): move to feature extractor (this function doesn't seem
	// fundamental)
	...
	/**
	 * If the formula represents a binary (e.g., fb:a.b.c or <=), return the ID of
	 * the binary as a string. If the formula represents a reversed binary (e.g.,
	 * !fb:a.b.c or (reverse fb:a.b.c)), return "!" + ID of the binary. Otherwise,
	 * return null.
	 */
	...
	public static Formula reverseFormula(Formula rawFormula) {
		if (rawFormula instanceof ValueFormula) {
			...
			// Convert (lambda x (relation1 (relation2 (var x)))) <=> (lambda x (!relation2
			// (!relation1 (var x))))
	...
	public static JoinFormula createItemsWithPropertyFormula(String property) {
