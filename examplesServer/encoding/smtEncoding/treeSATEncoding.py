...
class TreeSATEncoding:
    ...
    def __init__(self, D, testTraces): 
        ...
        defaultOperators = [encodingConstants.G, encodingConstants.F, encodingConstants.LNOT, encodingConstants.UNTIL, encodingConstants.LAND,encodingConstants.LOR, encodingConstants.IMPLIES, encodingConstants.X, 'prop']
    ...
    def generateRestrictions(self, level, positionAtLevel, positionAtTrace, traceId, trace):
        ...
        for operator in possibleOperators:
            if operator == encodingConstants.LNOT:
                self.solver.assert_and_track(Implies(possibleOperators[encodingConstants.LNOT], (currentNodeVariable == Not(leftChildAtSameTracePosition) ) ), 'negationOperator_'+str(currentNodeVariable) )
                
                #defining by default that the right child should be none (only one - left - child)
                self.solver.add(Implies(possibleOperators[encodingConstants.LNOT], leftChildAtSameTracePositionNoneConnector == False))
                self.solver.add(Implies(possibleOperators[encodingConstants.LNOT], rightChildAtSameTracePositionNoneConnector == True))
            ...
            elif operator == encodingConstants.LOR:
                self.solver.add(Implies(possibleOperators[encodingConstants.LOR], (currentNodeVariable == Or(leftChildAtSameTracePosition, rightChildAtSameTracePosition))))
            ...
            elif operator == encodingConstants.LAND:
                self.solver.add(Implies(possibleOperators[encodingConstants.LAND], (currentNodeVariable == And(leftChildAtSameTracePosition, rightChildAtSameTracePosition))))
                self.arityOfOperatorRestrictions(2, operator, leftChildAtSameTracePositionNoneConnector, rightChildAtSameTracePositionNoneConnector)
            
            elif operator == encodingConstants.IMPLIES:
                self.solver.add(Implies(possibleOperators[encodingConstants.IMPLIES], (currentNodeVariable == Implies(leftChildAtSameTracePosition, rightChildAtSameTracePosition))))
                self.arityOfOperatorRestrictions(2, operator, leftChildAtSameTracePositionNoneConnector, rightChildAtSameTracePositionNoneConnector)
                
            elif operator == encodingConstants.F:
                self.solver.add(Implies(possibleOperators[encodingConstants.F], currentNodeVariable == Or(futureNodeVariablesLeftChild) ))
                self.arityOfOperatorRestrictions(1, operator, leftChildAtSameTracePositionNoneConnector, rightChildAtSameTracePositionNoneConnector)
            
            elif operator == encodingConstants.G:
                self.solver.add(Implies(possibleOperators[encodingConstants.G], currentNodeVariable == And(futureNodeVariablesLeftChild)))
                self.arityOfOperatorRestrictions(1, operator, leftChildAtSameTracePositionNoneConnector, rightChildAtSameTracePositionNoneConnector)
            elif operator == encodingConstants.X:
                # there will always be at least two elements in futureNodeVariables... list (due to definition of futurePos function)
                self.solver.add(Implies(possibleOperators[encodingConstants.X], currentNodeVariable == futureNodeVariablesLeftChild[1]))
                self.arityOfOperatorRestrictions(1, operator, leftChildAtSameTracePositionNoneConnector, rightChildAtSameTracePositionNoneConnector)
            elif operator == encodingConstants.UNTIL:
                self.solver.add(Implies(possibleOperators[encodingConstants.UNTIL], currentNodeVariable == Or([\
