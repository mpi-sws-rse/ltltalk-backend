...
def transformIncomingJson(jsonInput, idStartedExperiment):    
    if jsonInput['q'].startswith("(:context"):
        return None
    
    id = jsonInput["sessionId"]
    if "start" in jsonInput["q"]:
        idStartedExperiment[id] = True
    if idStartedExperiment[id] == True:
        return jsonInput
    ...
    with open(queryLogFile) as inputFile:
        with open(outputFile, "w") as outputFile:
            for line in inputFile:
                transformedJson = transformIncomingJson(json.loads(line), idStartedExperiment)
